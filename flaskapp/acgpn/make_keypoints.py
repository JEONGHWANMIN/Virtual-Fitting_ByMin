...
def generate_pose_keypoints(file_path):
    modelpath = './acgpn/ACGPN_make_pose_model/'
    pose_model = general_pose_model(modelpath)

    res_points = pose_model.predict(file_path)

    file_name = file_path.split('/')[-1].split('.')[0]

    pose_data = {"version": 1.0,
                "people":  [
    ...
    print(file_name)
    pose_keypoints_path = f'./acgpn/Data_preprocessing/test_img/{file_name}_keypoints.json'
